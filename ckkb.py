...
class FakeDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        self.host_status_base = {
          ...
          'supported_instances': jsonutils.dumps([('x86_64', 'fake', None)]),
          'numa_topology': None,
          }
        self._mounts = {}
        self._interfaces = {}
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        def _wait_for_ready():
            ...
            if status == 'Success':
                LOG.info(_LI("[BAMPI] Task %(task_id)s ended successfully."),
                         {'task_id': t_id}, instance=instance)
        ...
        try:
            server = (s for s in servers
                            if s['hostname'].lower() == instance.hostname).next()
        except StopIteration:
            LOG.warn(_LW("[BAMPI] Server specified not found, hostname=%s"),
                     instance.hostname, instance=instance),
            server = None
            # TODO: Raise some exception to upper layer

        # Normal case (VM spawning)
        if server is None:
            return
        ...
        try:
            ...
            # If we cannot find 'id' in returned json...
            LOG.error(_LE("[BAMPI] RESP => ret_code=%s, image %s not found"),
                      r.status_code, image_meta['name'], instance=instance)
            return
        else:
        ...
        timer = loopingcall.FixedIntervalLoopingCall(_wait_for_ready)
        ret = timer.start(interval=5).wait()

        # FIXME: Currently do nothing with failed tasks
        if ret == False:
            #pass
    ...
    def power_off(self, instance, shutdown_timeout=0, shutdown_attempts=0):
        ...
        try:
            ...
            self.instances[instance['name']].state = power_state.SHUTDOWN

    def power_on(self, context, instance, network_info, block_device_info):
        LOG.info(_LI("[BAMPI] Power on hostname=%s" % instance.hostname),
                 instance=instance)
        try:
            r = requests.put("http://{bampi_ip_addr}:{bampi_port}{bampi_api_base_url}/servers/{hostname}/powerStatus"
                                     .format(bampi_ip_addr=BAMPI_IP_ADDR,
            ...
            self.instances[instance['name']].state = power_state.RUNNING
    ...
    def get_available_nodes(self, refresh=False):
        ...
        for server in response.json():
            i = "id={id}, hostname={hostname}, " \
                ...
                "bmb_ip_addr={bmc[ipAddress]}".format(**server)
            info += i
